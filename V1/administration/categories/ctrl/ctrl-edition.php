...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $categorie = valid_donnees(trim($_POST['idCategorie']));


    //ON MET A JOUR LE JEU
    $sqlInfosJeu = $bdd -> prepare("UPDATE categories SET nom= :nom, urlNom = :urlNom WHERE idCategorie = :categorie");
    $sqlInfosJeu-> execute(array("nom" => $nom, "urlNom" => $urlNom, "categorie" => $categorie));
    ...
    if(isset($_FILES["photo"]) && $_FILES["photo"]["size"] > 0){
        if($_FILES["photo"]["error"] == 0){
            ...
            if(in_array($filetype, $allowed)){
                ...
                $sqlImageExiste = $bdd -> query("SELECT * FROM categories_image WHERE idCategorie =". $categorie);
                ...
                if($nbrImage < 1){
                    ...
                    $sqlImage = $bdd -> prepare("UPDATE categories_image SET image = ?, image_type = ? WHERE idCategorie = ?");
                    $sqlImage-> execute(array($imgBase64,$filetype,$categorie));
                }
    ...
    $_SESSION['alertMessage'] = "Catégorie mise à jour !";
