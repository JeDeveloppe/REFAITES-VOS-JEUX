        ...
        <div class="card col-xl-9 mx-auto p-0">
            ...
            <div class="card-body">
                <form method="get" action ="" class="d-flex">
                    <div class="form-group text-center">Type de document:
                    ...
                    <div class="form-group text-center">Numéro du document:
                        <input type="text" name="numero" placeholder="SANS LES LETTRES..." pattern="[0-9]{2,8}"/>
                    </div>
                    <div class="form-group text-center">Nom du client:
                        <input type="text" name="nom" maxlength="20" placeholder="MAX 20 caractères..."/>
                    </div>
                ...
                <div class="card-body p-0">
                    ...
                    if(!isset($_GET['type'])){
                        ...
                        if($_GET['nom'] != ""){
                            ...
                            $textNom = "AND idUser IN(SELECT idClient FROM clients WHERE nom LIKE '%$nom%')";
                        }else{
                            $nom = "";
                            $textNom = '';
                        }
                        ...
                        $sqlRecherche = $bdd -> query("SELECT * FROM documents WHERE $textEtat $textNumero $textNom");
                        $nb = $sqlRecherche -> rowCount();
                            ...
                            if($nb < 1){
                                ...
                                while($donneesRecherche){
                                    ...
                                    echo '<tr>
                                            <td>'.$donneesClient['idClient'].'</td>
                                            <td>'.$donneesClient['nom'].' '.$donneesClient['prenom'].'<br/>'.$donneesClient['adresse'].'<br />'.$donneesClient['cp'].' '.$donneesClient['ville'].'</td>
                                            <td>';
                                            ...
                                            echo '</td>
                                            <td>'.$donneesRecherche['numero_devis'].'<br/> Créé le '.date('d.m.Y',$donneesRecherche['time']).'<br/>';
                                                if($donneesRecherche != ""){
                                            ...
                                            echo '</td>
                                            <td>'.$donneesRecherche['totalTTC'].'</td>
                                            <td>';
                                            ...
                                            <td>';
                                                if($donneesRecherche['numero_facture'] != ""){
                                                echo '<a href="/administration/facture/generation-pdf-envoi.php?document='.$donneesRecherche['idDocument'].'" class="btn btn-primary">Facture par email</a><br />
                                                        <a href="/administration/facture/generation-pdf.php?document='.$donneesRecherche['idDocument'].'" target="_blank" class="btn btn-warning border-primary mt-2">Facture en pdf</a>';
                                                }else{
                                                    echo '
                                                    <a href="/admin/devis/edition/'.$donneesRecherche['numero_devis'].'" class="btn btn-warning">Voir et modifier</a>
                                                    <a href="/admin/devis/delete/'.$donneesRecherche['numero_devis'].'" class="btn btn-danger">Voir et supprimer</a>
                                                    <form method="POST" action="/administration/comptabilite/saisie-paiement-manuelle.php" class="mt-3">
                                                        <div class="form-group">
                                                            ...
                                                            <button type="submit" class="btn btn-info">Saisir un paiement manuel</button>
