...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(isset($_FILES["photo"]) && $_FILES["photo"]["size"] > 0){
        if($_FILES["photo"]["error"] == 0){
            ...
            if(in_array($filetype, $allowed)){
                ...
                $sqlImageExiste = $bdd -> query("SELECT * FROM jeu_image WHERE idJeux =". $jeu);
                ...
                if($nbrImage < 1){
                    ...
                    $sqlImage = $bdd -> prepare("UPDATE jeu_image SET image = :image, image_type = :type WHERE idJeux = :jeu");
                    $sqlImage-> execute(array("image" => $imgBase64, "type" => $filetype ,"jeu" => $jeu));
                }
    ...
    $_SESSION['alertMessage'] = "Ok mis à jour !";
