...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(!preg_match('#^[0-9]{8,14}$#', $telephone)){
    ...
    require("../../../bdd/connexion-bdd.php");
      ...
      try{
    $sqlSaveClient = $bdd -> prepare("UPDATE clients SET nom = ?, prenom = ?, adresse = ?, cp = ?, ville = ?, pays = ?, telephone = ?, email = ? WHERE idClient = ?");
    $sqlSaveClient-> execute(array($nom,$prenom,$adresse,$cp,$ville,$pays,$telephone,$email,$id));
    }
    catch (Exception $e) {
