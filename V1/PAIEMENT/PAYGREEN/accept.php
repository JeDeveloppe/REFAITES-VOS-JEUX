...
if(!isset($_GET['doc']) || !isset($_GET['user'])){
    ...
    if($count != 1){
        ...
        $data = array(
            ...
            "returned_url" => $GLOBALS['domaine']."/paiement/PAYGREEN/finDeTransaction-valide.php",                        //url retour client apres transaction
            //"notified_url" => $GLOBALS['domaine']."/paiement/PAYGREEN/notificationPaiement.php",                        //url de notification vers serveur
        ...
        curl_close($ch);
            ...
            if(isset($result)){
                ...
                if($objet['success'] == "true"){ //echange OK on redirige vers l'url de paiement
                    ...
                    // A RECUPERER SUR PAGE VALIDATION
                    $_SESSION['payment_id'] = $transaction;
                ...
                $_SESSION['alertMessage'] = "Échec de connexion à l'API PAYGREEN !<br/> Veuillez réessayer !";
                $_SESSION['alertMessageConfig'] = "warning";
                header("Location: /");
                exit(); 
            }
