    ...
    if(empty($_POST['recaptcha-response'])){
        ...
        if(empty($response) || is_null($response)){
            ...
            if($data->success){
                ...
                //PREPARATION V2
                $sqlVerifClientUnique = $bdd->prepare("SELECT * FROM clients WHERE email = ? AND telephone = ? AND port = ? AND optionExpedition = ?");
                $sqlVerifClientUnique->execute(array($_SESSION['email'],$_SESSION['telephone'],$_SESSION['port'],$_SESSION['expeditionOption']));
                $donneesVerifClientUnqiue = $sqlVerifClientUnique->fetch();
                $nbrClientUnique = $sqlVerifClientUnique->rowCount();

                //on recupere tout ce qui est en attente...
                ...
                if($nbrClientUnique != 1){
                    //si adresse email non unique on fait comme avant...
                    ...
                    //si adresse email unique dans la base client...
                    $sqlListeMessagesUpdateDevis = $bdd -> prepare("UPDATE listeMessages SET idUser = ?, statut = 1 WHERE idUser = ?");
                    $sqlListeMessagesUpdateDevis-> execute(array($donneesVerifClientUnqiue['idUser'],$_SESSION['sessionId']));
                }
