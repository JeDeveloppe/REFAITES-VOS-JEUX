...
$totalAnnuel = 0;
    for($m=1;$m<=12;$m++){
        $sqlGraph = $bdd->prepare("SELECT * FROM clients WHERE MONTH(FROM_UNIXTIME(isAssociation)) = ? AND YEAR(FROM_UNIXTIME(isAssociation)) = ?");
        $sqlGraph->execute(array($m,$annee+1));
        $donneesGraph = $sqlGraph->fetch();
        $donneesGraph_100 = $sqlGraph->rowCount();
