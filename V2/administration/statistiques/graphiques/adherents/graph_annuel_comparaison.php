...
$totaux1y = [];
    for($m=1;$m<=12;$m++){
        $sqlGraph = $bdd->prepare("SELECT SUM(isAssociation) as totalHTmois FROM clients WHERE MONTH(FROM_UNIXTIME(time_transaction)) = ? AND YEAR(FROM_UNIXTIME(time_transaction)) = ?");
        $sqlGraph->execute(array($m,$anneePassee+1));
        $donneesGraph = $sqlGraph->fetch();
        $donneesGraph_100 = $donneesGraph['totalHTmois'];
...
$totaux2y = [];
    for($m=1;$m<=12;$m++){
        $sqlGraph2 = $bdd->prepare("SELECT SUM(isAssociation) as totalHTmois FROM clients WHERE MONTH(FROM_UNIXTIME(time_transaction)) = ? AND YEAR(FROM_UNIXTIME(time_transaction)) = ?");
        $sqlGraph2->execute(array($m,$anneeN+1));
        $donneesGraph2 = $sqlGraph2->fetch();
        $donneesGraph2_100 = $donneesGraph2['totalHTmois'];
...
$graph->title->Set("Adhérents en cours de validitée");
