...
$totaux1y = [];
    for($m=1;$m<=12;$m++){
        ...
        WHERE d.etat = 2 AND c.deee = ? AND MONTH(FROM_UNIXTIME(d.time_transaction)) = ? AND YEAR(FROM_UNIXTIME(d.time_transaction)) = ?");
        $sqlJeuxOccasionVendus->execute(array("OUI",$m,$anneePassee));
...
$totaux2y = [];
    for($m=1;$m<=12;$m++){
        ...
        LEFT JOIN documents d ON d.idDocument = dd.idDocument
        WHERE d.etat = 2 AND c.deee = ? AND MONTH(FROM_UNIXTIME(d.time_transaction)) = ? AND YEAR(FROM_UNIXTIME(d.time_transaction)) = ?");
        $sqlJeuxOccasionVendus->execute(array("OUI",$m,$anneeN));
        $donneesDuMois = $sqlJeuxOccasionVendus->fetch();
...
$graph->title->Set("Grammes (DEEE) par mois ".$anneePassee." - ".$anneeN);
