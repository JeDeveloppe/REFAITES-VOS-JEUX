...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $urlNom = clean_url($nom);


    //ON CRE DANS LA BASE DE DONNEE
    $sqlNewAccessoire = $bdd -> prepare("INSERT INTO catalogue (nom,urlNom,accessoire_idCategorie,actif) VALUES (?,?,?,?)");
    $sqlNewAccessoire-> execute(array($nom,$urlNom,$categorie,0));

    //on recupere dernier entree
    $accessoire = $bdd->lastInsertId();

        //ON CREE LA LIGNE POUR LE CONTENU DES PIECES
        $sqlUpdatePiece = $bdd -> prepare("INSERT INTO pieces (idJeu,contenu_total) VALUES (?,?)");
        $sqlUpdatePiece-> execute(array($accessoire,$description));
    ...
    if(isset($_FILES["photo"]) && $_FILES["photo"]["size"] > 0){
        if($_FILES["photo"]["error"] == 0){
            ...
            if(in_array($filetype, $allowed)){
                ...
                $sqlImageCreate-> execute(array($accessoire,$imgBase64,$filetype));
    ...
    header("Location: /admin/accessoire/".$accessoire."/edition/");
    exit();  
