...
$sqlDocument = $bdd->prepare("SELECT * FROM documents WHERE idUser = ? AND etat = 2 OR (idUser = ? AND etat < 2 AND end_validation >= ?) ORDER BY idDocument DESC");
$sqlDocument-> execute(array($_SESSION['idClient'],$_SESSION['idClient'],time()));
                        ...
                        if($doc['numero_facture'] != ""){
                            echo '<div class="card mb-2">
                                    <div class="card-header d-flex flex-wrap align-items-center">
                                        ...
                                        <div class="col-12 col-lg text-right"><a href="/membre/pdf/?document='.$doc['validKey'].'" target="_blank" class="btn btn-info border-primary"><i class="fas fa-print"></i> Facture en pdf</a></div>
                                    </div>
                                </div>';
                        }else{
