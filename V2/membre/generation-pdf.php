...
if($_SERVER['REQUEST_METHOD'] == 'GET'){
    ...
    $sqlDocument = $bdd -> prepare("SELECT * FROM documents WHERE validKey = ? AND idUser = ?");
    $sqlDocument-> execute(array($document, $_SESSION['idClient']));
    ...
    $sqlDetailsDocumentsAchats->execute(array($donneesDocument['idDocument']));
    ...
    $pdf->SetX(8);
    //si version 1 ou version 2
    if($donneesDocument['time_transaction'] < 1646089200){
        $pdf->MultiCell(168,8,utf8_decode("Préparation:"),1,'R');
    }else{
