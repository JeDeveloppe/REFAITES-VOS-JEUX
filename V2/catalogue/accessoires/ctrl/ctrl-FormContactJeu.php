                        ...
                        if($_FILES["photo"]["size"][$i] > 0){
                            if($_FILES["photo"]["error"][$i] == 0){
                                ...
                                if($filesize[$i] > $maxsize){
                                    $_SESSION['alertMessage'] = "Image n° ".[$i]." trop grande, maximum ".$tailleImage."MB (ou ".$maxsize." octets)!";
                                    $_SESSION['alertMessageConfig'] = "warning";
                                    header("Location: ".$_SERVER['HTTP_REFERER']);
                                    exit();
                                }
        ...
        if(empty($_POST['recaptcha-response'])){
            ...
            if(empty($response) || is_null($response)){
                ...
                if($data->success){
                    ...
                    $sqlListeMessages = $bdd -> prepare("INSERT INTO listeMessages (idUser, idJeu, message, time, statut) VALUES (:user, :jeu, :message, :creation, :statut)");
                    $sqlListeMessages-> execute(array("user" => $_SESSION['sessionId'], "jeu" => $idDuJeu, "message" => $content, "creation" => time(), "statut" => 0));
                    ...
                    $_SESSION['alertMessage'] = "Message ajouté à mes demandes !<br/>N'oubliez pas de valider votre liste !";
